...
ScopeStack *GMAM::scopes = &__global_scope_stack;

typedef std::vector<Scope *> stk_t;
...
Symbol *ScopeStack::lookup(std::string id, Location *loc, bool through) {
    ...
    if (through) {
        ...
        return _stack.back()->lookup(id, loc);
    }
}
...
void ScopeStack::declare(Symbol *s) {
    assert(NULL != s && !_stack.empty());

    _stack.back()->declare(s);
}
...
void ScopeStack::open(Scope *sco) {
    ...
    case Scope::GLOBAL:
        ...
        break;
    case Scope::FUNCTION:
        // GMAM_assert(! _stack.empty());
        break;
    default:
        break;
    }

    _stack.push_back(sco);
}
